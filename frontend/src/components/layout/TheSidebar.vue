...
<template>
  <div class="h-[100%] p-5 w-[300px] fixed bg-black">
    ...
    <ul v-else-if="status === 'success'">
      <li v-for="playlist in data" :key="playlist.id" class="flex justify-between items-center gap-1.5 sidebar-item cursor-pointer">
        <router-link :to="{ name: 'playlist', params: { id: playlist.id } }" class="flex items-center gap-1.5">
